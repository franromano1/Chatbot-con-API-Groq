# iniciar:   python -m streamlit run main.py
...
MODELOS = ["llama-3.1-8b-instant",
           "llama-3.3-70b-versatile", "llama-guard-4-12b"]
...
def configurar_pagina():
    ...
    st.set_page_config(page_title="Fran GPT", page_icon="🐒")
    st.title("La mejor IA de todas: Fran GPT 🐒")
...
# GUARDA LO NUEVO EN EL HISTORIAL Y LE PONE UN ROL Y LOGO AL EMISOR
...
# FILTRA LA RESPUESTA COMPLETA DEL BOT PARA MOSTRAR LO QUE VERÁ EL USUARIO
...
def main():
    ...
    if prompt_del_usuario:
        ...
        respuesta_del_bot = configurar_modelo(
            cliente_usuario, modelo_elegido_por_el_usuario, prompt_del_usuario)

        if respuesta_del_bot:
            with st.chat_message("assistant"):
                respuesta_posta = st.write_stream(
                    generar_respuesta(respuesta_del_bot))
